  ...
  <div class="table-box">
    <ProTable useOrgani ref="proTable" title="操作日志" :columns="columns" :requestApi="getTableList" :row-key="`log_id`">
      ...
      <template #expand="scope">
        <div class="expand">
          <div class="expand-right break-word">
            <div>请求页面:</div>
            <div class="params-content">
              <div class="expand-params interval">
          ...
          <div class="expand-right break-word">
            <div>sql:</div>
            <div class="params-content">
              <div v-for="(item, index) in scope.row.sql" :key="index" class="expand-sql interval">
                <div>语句:&nbsp;&nbsp;{{ item.sql }}</div>
                <div>参数:&nbsp;&nbsp;{{ item.params }}</div>
                <div>执行时长:&nbsp;&nbsp;{{ item.time }}（ms）</div>
              </div>
            </div>
          </div>
        </div>
...
<script setup lang="tsx" name="DevOperationLogIndex">
import { ref } from "vue";
import { ColumnProps } from "@/components/ProTable/interface";
import ProTable from "@/components/ProTable/index.vue";
import { operationLogList } from "@/api/modules/develop";
...
const getTableList = async (params: Develop.OperationLog.PageReq) => {
  if (params.operation_time) params.operation_time = getDateTime(params.operation_time);
  return operationLogList(params);
};
...
const columns: ColumnProps[] = [
  ...
  { type: "expand", label: "其他信息", width: 100 },
