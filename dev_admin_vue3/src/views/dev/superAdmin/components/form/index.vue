...
import { superAdminEdit, superAdminAdd } from "@/api/modules/develop";
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      if (data.admins_id !== 0) {
        ...
        await superAdminEdit({ ...form, admins_id: data.admins_id } as RuleForm<number>);
        ElMessage.success("编辑成功");
      } else {
        await superAdminAdd(ruleForm as RuleForm<undefined>);
 ...
 * 对外暴露子组建方法
 */
const open = (params: RuleForm<number> | undefined = undefined) => {
