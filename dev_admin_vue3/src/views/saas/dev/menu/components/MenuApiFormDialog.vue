...
import { menuApiAdd, menuApiEdit } from "@/api/modules/saasDevelopMenu";
...
const rules = reactive<FormRules>({
  ...
  api_name: [
    { required: true, message: "接口名称不为空", trigger: "blur" },
    {
      max: 255,
      message: "不能超过255字符",
      trigger: "blur"
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      if (data.api_id !== 0) {
        await menuApiEdit({ ...ruleForm, api_id: data.api_id });
        ElMessage.success("编辑成功");
      } else {
        await menuApiAdd(ruleForm);
 ...
 * 对外暴露子组建方法
 */
const open = (menu_id: number, params: RuleForm | null = null) => {
  if (params !== null) {
