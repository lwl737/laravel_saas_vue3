...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.path.toLocaleLowerCase() === LOGIN_URL) {
    if (userStore.tokenGet) return next(from.fullPath);
    resetRouter();
    return next();
  }
  ...
  if (!userStore.tokenGet) return next({ path: LOGIN_URL, replace: true });
    ...
    authStore.setAuthMenuList(res.data.menu, res.data.role_id); //设置栏目权限
