...
const initData = async (first: boolean = false, value: number | string | undefined = undefined) => {
  if (props.multiple) selected.value = Array.isArray(props.defaultValue) ? props.defaultValue : [props.defaultValue];
  else selected.value = typeof value !== "undefined" ? value : props.defaultValue;
  loading.value = true;
  const { data } = await props.requestApi!();
  // console.log(data);
  treeData.value = data;
  treeAllData.value = props.showAll === true ? [{ [props.id]: "", [props.label]: "全部" }, ...data] : data;
