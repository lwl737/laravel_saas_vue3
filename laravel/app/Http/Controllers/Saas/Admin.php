...
use App\Services\Saas\{AdminsLoginInfo, Permissions};
use App\Helpers\Inject;
use App\Dao\Mysql\Saas\RoleDao\Orm as RoleDao;
use App\Dao\Mysql\Saas\RoleDao\Enums\RoleEnable as RoleEnable;
use App\Dao\Mysql\Saas\OrganiAdminsBindDao\Orm as OrganiAdminsBindDao;
...
class Admin extends BaseController
{
    ...
    public function login()
    {
        ...
        //账号密码错误
        if (!$res) return $this->outputEnum(Saas::USER_PASS_ERROR);

        //账号被禁用
        if (!$res->isEnable()) return $this->outputEnum(Saas::USER_DISABLE);

        if ($res->role_id > 0) {
            $role_res =  RoleDao::static()->checkRoleEnable($res->toArray());
            if ($role_res instanceof RoleEnable) return match ($role_res) {
                //权限不存在
                RoleEnable::NOT_EXIST => $this->outputEnum(Saas::ROLE_NOT_EXIST),
                //权限被禁用
                RoleEnable::STATUS_NOT_ENABLE => $this->outputEnum(Saas::ROLE_DISABLE),
                default => $this->outputEnum(Saas::LOGIN_TOKEN_ERROR)
            };

            $organi_id = OrganiAdminsBindDao::static()->adminsOrganiIdFirst($res->admins_id);

            //组织不存在
            if (!$organi_id) return $this->outputEnum(Saas::ORGANI_NOT_EXIST);
        } else $organi_id = -1;

        return  $this->output(data: ["organi_id" => $organi_id], http: Success::LOGIN_SUCCESS, header: AdminsLoginInfo::createToken([...$res->toArray() , 'tenant_id' => Inject::get(AdminsLoginInfo::class)->getTenantId()]));
    }
    ...
    public function editUserInfo()
    {
        ...
        ) return $this->outputEnum(Saas::USERNAME_IS_EXIST);
    ...
    public function editPass()
    {
        ...
        if (!$res) return $this->outputEnum(Saas::ADMINS_UPDATE_PASS_ERROR);
    ...
    public function list()
    {
        ...
        if (!empty($organi_link)) {
            if( isset( $this->params["organi_id"] ) && !$this->checkOrgani( $this->params["organi_id"] , false )  ) return $this->outputEnum( Saas::ORGANI_NOT_EXIST);
    ...
    {
        //判断用户名是否重复
        if (AdminsDao::static()->checkUsername(["username" => $this->params["username"]])->query()->exists()) return  $this->outputEnum(Saas::USERNAME_IS_EXIST);

        $role = RoleDao::static()->organiScope()->checkRoleId(["role_id" => $this->params["role_id"]])->query()->first(['status']);
        //权限不存在
        if (!$role) return $this->outputEnum(Saas::ROLE_NOT_EXIST);
        //权限已被禁用禁用
        if (!$role->isEnable()) return $this->outputEnum(Saas::ROLE_DISABLE);
        //检查部门 排除自己部门 添加和编辑时都要做检查
        if (!$this->checkOrgani($this->params["organi_id"])) return $this->outputEnum(Saas::ORGANI_ERROR);
    ...
    public function edit()
    {
        ...
        ], $this->params["admins_id"])->query()->exists()) return  $this->outputEnum(Saas::USERNAME_IS_EXIST);
