...
namespace App\Dao\Mysql\Saas\AdminsDao;

use App\Models\Mysql\Orm\Saas\Admins;
use App\Models\Mysql\Orm\Saas\AdminsDel;
...
{
    public function __construct(int|string|null $tenant_id = null)
    {
        if($tenant_id) Server::tenancyInitialize($tenant_id);
    ...
    {
        $total = (clone $this->query)->groupBy(DB::raw("admins.admins_id WITH ROLLUP"))
            ->orderBy('total', 'desc')
            ->limit(1)
            ->first([DB::raw(" COUNT( DISTINCT `admins`.`admins_id` ) as `total` ")])?->total;
    ...
    public function getList(array $params)
    {
        ...
        return $this->list($params, beforeFunc: fn ($query) =>  $query->orderBy('role_sort', 'desc'))->query()->get([
            ...
            'admins.role_id',
            'admins.portrait',
            DB::raw('MAX(`role`.`role_name`) as role_name'),
            DB::raw("GROUP_CONCAT(organi.organi_name SEPARATOR '\n') as organi_name"),
            DB::raw("GROUP_CONCAT(organi.organi_id) as organi_id")
        ])->map(function ($item) {
            $organi_id = [];
