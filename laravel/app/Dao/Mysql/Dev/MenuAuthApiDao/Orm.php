...
namespace App\Dao\Mysql\Dev\MenuAuthApiDao;
use App\Dao\Mysql\Dev\BaseDev;
use App\Models\Mysql\Orm\Dev\MenuAuthApi;
...
class Orm extends BaseDev{
    ...
    public function checkApi(array $params){
        ...
        $this->query = $this->query->leftJoin("menu_auth","menu_auth.auth_id","=","menu_auth_api.auth_id")->where('menu_auth_api.menu_id' , $params['menu_id'])->where('menu_auth_api.api',$params['api']);

        if(isset($params["api_id"]))  $this->query = $this->query->where('menu_auth_api.api_id' , "<>" , $params["api_id"]);
        ...
        $res = $this->query->first(["menu_auth.auth_name" , "menu_auth_api.api_name"]);
