...
namespace App\Dao\Mysql\Dev\SaasMenuDao;
...
class Orm extends BaseDev{
    ...
    public function add(array $params , int $maxLevel , array $ancestor_link , array $level_link){
        ...
        DB::transaction(function () use ($ancestor_link, $maxLevel , $level_link) {
            ...
            (new SaasMenuRelation)->insertAll($ancestor_link, [], fn ($item , $index ) => ["menu_ancestor_id" =>  $item, "menu_descendant_id" => $menu_id, "level" =>  $index >= count($level_link) ? $maxLevel + 1 : $level_link[$index] ]);
        });
    }
