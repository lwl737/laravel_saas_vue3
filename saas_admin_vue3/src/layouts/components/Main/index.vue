...
import { ref, onBeforeUnmount, provide, watch, h, onMounted } from "vue";
...
const elMains = ref<any>();

onMounted(() => {
  mittBus.on("scrollTo", scrollTop);
});

const scrollTop = (option: { y: number }) => {
  if (elMains.value?.$el) elMains.value.$el.scrollTop = option.y;
};
...
onBeforeUnmount(() => {
  ...
  mittBus.off("scrollTo", scrollTop);
});
</script>
