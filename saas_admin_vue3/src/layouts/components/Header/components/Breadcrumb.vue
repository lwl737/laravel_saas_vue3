...
const breadcrumbList = computed(() => {
  ...
  // 🙅‍♀️不需要首页面包屑可删除以下判断
  if (breadcrumbData[0].path !== homeUrl.value) {
    breadcrumbData = [{ path: homeUrl.value, meta: { icon: "HomeFilled", title: "首页" } }, ...breadcrumbData];
  }
  return breadcrumbData;
});

const homeUrl = computed(() => {
  return "/" + userStore.tenantIdGet + HOME_URL;
});
